...
public class TakeCustomer implements Command {
    @Override
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        if (message.isFromType(ChannelType.TEXT)) {
            if (message.getMentionedUsers().isEmpty()) {
                channel.sendMessage(tagsender + " :poodle: Je moet een of meerdere leden taggen om de klanten rank af te nemen.").queue();
            } else {
                ...
                for (User user : mentionedUsers) {
                    ...
                    guild.getController().removeRolesFromMember(member, event.getGuild().getRoleById(STATIC.ROLE_CUSTOMER_ID)).queue(
                            success -> channel.sendMessage(tagsender + " , ").append(member.getAsMention()).append(" zit niet langer in de klanten rank").queue(),
                            error ->
                            {
