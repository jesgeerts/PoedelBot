...
event.getMessage().delete().queue();
        ...
        .replace("%minecraftsupport%", event.getJDA().getTextChannelById("398419699341197322").getAsMention())
                .replace("%mcnetwork%", event.getJDA().getTextChannelById("398419736993464322").getAsMention())
                .replace("%panelsupport%", event.getJDA().getTextChannelById("398419783198179328").getAsMention())
                .replace("%webhostingsupport%", event.getJDA().getTextChannelById("417673629262938122").getAsMention())
                .replace("%botcommands%", event.getJDA().getTextChannelById("398421115925561354").getAsMention())
                + ("\nVoor de prijzen kan je hier terecht: %prijzen%\nEn voor alles wat niet support hoort en gewoon gezellige praat/discussie is hoort in %offtopic%")
                .replace("%offtopic%", event.getJDA().getTextChannelById("404964779263000579").getAsMention())
                .replace("%prijzen%", event.getJDA().getTextChannelById("399880740680237067").getAsMention())).build()).queue();
        channel.sendMessage(new EmbedBuilder().setColor(Color.cyan).setDescription("Het is sindskort mogelijk om ook tickets te maken. Gebruik ```-new``` of ```-ticket``` om een ticket te maken. We vragen wel aan je om die commando's niet te spammen aangezien dit tot een ban of een kick kan leiden.").setTitle("Tickets").build()).queue();
