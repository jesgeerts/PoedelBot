    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        if(member3.hasPermission(Permission.MANAGE_ROLES) || member3.hasPermission(Permission.MANAGE_PERMISSIONS) || member3.hasPermission(Permission.ADMINISTRATOR)){
            ...
            if (message.getMentionedUsers().isEmpty()) {
                channel.sendMessage(tagsender + " :poodle: Je moet een of meerdere leden taggen om de persoon te muten :poodle:").queue();
            } else {
                ...
                for (User user : mentionedUsers) {
                    ...
                    guild.getController().addSingleRoleToMember(member, event.getGuild().getRoleById(STATIC.ROLE_MUTED_ROLE)).queue(
                            success -> channel.sendMessage(tagsender + " , ").append(member.getAsMention()).append(" is nu gemuted").queue(),
                            error ->
                            {
