...
public class UnbanID implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        if (member3.hasPermission(Permission.ADMINISTRATOR) || member3.hasPermission(Permission.BAN_MEMBERS)) {
            ...
            if (!selfMember.hasPermission(Permission.BAN_MEMBERS) || !selfMember.hasPermission(Permission.ADMINISTRATOR)) {
                channel.sendMessage(tagsender + " HE POEDEL! IK HEB GEEN TOESTEMMING OM POEDELS TE UNBANNEN").queue();
                return; //We jump out of the method instead of using cascading if/else
            }


            guild.getController().unban(args[0]).queue(

                    success -> channel.sendMessage("De gebruikersID ``").append(args[0]).append("`` Is niet langer verbannen van %server%".replace("%server%", event.getGuild().getName())).queue()
