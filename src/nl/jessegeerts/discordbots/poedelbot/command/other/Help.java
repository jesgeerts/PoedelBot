        ...
        if (args.length == 0) {
          ...
          /**
            ...
            channel.sendMessage(embedBuilder
                    .setDescription("**Hosting commands**\n\n.website - Vertelt je onze website\n.prijzen - Stuurt je door naar onze %prijzen% (#prijzen) kanaal".replace("%prijzen%", event.getGuild().getTextChannelById(STATIC.CHANNEL_PRICES_ID).getAsMention()))

                    .build()).queue();

            channel.sendMessage(embedBuilder.setTitle("**Moderatie commands**").setDescription(".pin - Zet een bericht vast (.pin BERICHTID)\n.unpin - Verwijder bericht uit pins (.unpin BERICHTID)\n.kick - Schop een poedel uit de server (.kick @Mention1 @Mention2)\n.ban - Verban een poedel uit de server (.ban @Mention1 @Mention2)\n.clear - Verwijder een aantal berichten uit de chat. Dit kan vanaf 2 (tot 100) berichten en je moet je eigen clear bericht meetellen. (.clear 50)").build()).queue();
            channel.sendMessage(embedBuilder.setTitle("**Administrator/Owner commands**").setDescription(".givecustomer - Geef een klant zijn klanten rank (.givecustomer @Mention1 @Mention2)\n.talk - Laat de bot praten :open_mouth: (.talk JE BERICHT)\n\n\n***De rest wordt nog toegevoegd. Onze excuses voor het ongemak dat de lijst niet compleet is***").build()).queue();
            channel.sendMessage(embedBuilder.setTitle("Pret Commands").setDescription(".dikkiedik - Maak iemand voor Dikkie Dik uit door middel van een mention\n.dog - Post memedog zonder het doorstuur gedeelte\n.dora - Dora klik op dora! #RackCity!\n.eightball - Laat de beroemde Eightball een voorspelling doen\n.gas - GEEF GAS!\n.happy - Wees net zo blij als Barrack Obama.\nJoke - Stuurt een leuke grap\n.koekoek - KOEKOEK JONGUH\n").build()).queue();
